...
for(pred_in_d in pred_in_d_opt){
  ...
  for(pred_task in pred_task_lst){
    ...
    for(fs_type in fs_type_opt){
      ...
      write_svmlight(x = train_mt, y=y_tr_sp$y, file = "./data/train_svmlite.txt", zero_based = FALSE)
      write_svmlight(x = test_mt, y=y_ts_sp$y, file = "./data/test_svmlite.txt", zero_based = FALSE)
