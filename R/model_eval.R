...
data_proc<-readRDS("~/AKI/AKI_Discover_Cohort/data/data_proc.rda")
tbl1<-data_proc$cohort %>%
  dplyr::select(ENCOUNTERID,SERUM_CREAT_BASE) %>%
  left_join(data_proc$demo %>%
              filter(key=="AGE") %>%
...
for(pred_in_d in pred_in_d_opt){
  ...
  for(pred_task in pred_task_lst){
    ...
    for(fs_type in fs_type_opt){
      ...
      valid_orig<-readRDS(paste0("./data/model_ref/",pred_in_d,"d_",fs_type,"_",pred_task,".rda"))$valid %>%
