...
for(pred_in_d in pred_in_d_opt){
  ...
  for(pred_task in pred_task_lst){
    ...
    X_tr<-dat_ds[[pred_task]][[2]][["X_surv"]] %>%
      semi_join(dat_ds[[pred_task]][[1]] %>% filter(cv10_idx<=7 & yr<2017),
                by="ENCOUNTERID")
    
    y_tr<-dat_ds[[pred_task]][[2]][["y_surv"]] %>%
      inner_join(dat_ds[[pred_task]][[1]] %>% filter(cv10_idx<=7 & yr<2017),
                 by="ENCOUNTERID")
    
    X_ts<-dat_ds[[pred_task]][[2]][["X_surv"]] %>%
      semi_join(dat_ds[[pred_task]][[1]] %>% filter(cv10_idx>7 | yr>=2017),
                by="ENCOUNTERID")
    
    y_ts<-dat_ds[[pred_task]][[2]][["y_surv"]] %>%
      inner_join(dat_ds[[pred_task]][[1]] %>% filter(cv10_idx>7 | yr>=2017),
                 by="ENCOUNTERID")
    ...
    for(fs_type in fs_type_opt){
      ...
      clusterExport(cl,c('folds','X_tr_sp','y_tr_sp')) # copying data to clusters
      clusterEvalQ(cl,expr= {                          # copying model to clusters
        library(xgboost)
      })
      
      #--tune hyperparameter (less rounds, early stopping)
      xgb_cv_bayes <- function(max_depth=10L, min_child_weight=1L, subsample=0.7, feature_set=NULL,
                               eta=0.05,colsample_bytree=0.8,lambda=1,alpha=0,gamma=1) {
        
        if(!is.null(feature_set)){
          #build a quick initial model with large step size, less deep trees
          init_xgb<-xgboost(data = dtrain, 
                            max.depth = round(log2(ncol(dtrain)))*2,
                            eta = 0.5, nrounds = 5, colsample_bytree=0.5,
                            objective = "binary:logistic")
          
          dtrain<-xgb.DMatrix(data=X_tr_sp[,xgb.importance(model=init_xgb) %>%
                                             dplyr::slice(1:round(feature_set)) %>% select(Feature) %>% unlist],
                              label=y_tr_sp$y)
        }else{
          dtrain<-xgb.DMatrix(data=X_tr_sp,
                              label=y_tr_sp$y)
        }
